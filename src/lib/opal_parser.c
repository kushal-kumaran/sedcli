...
int opal_parser_init(void)
{
    ...
    for (int i = 0; i < OPAL_MAX_TOKENS; i++) {
...
int append_bytes(uint8_t *buf, size_t len, const uint8_t *src, size_t src_len)
{
    int ret = 0;
        ...
        if (curr_byte & OPAL_ATOM_SHORT_SIGNED)
            ...
            if (token->len <= 9) {
                for (i = token->len - 1; i > 0; i--) {
                    token->vals.uint |= (uint64_t)buf[i] << (8 * idx);
                    idx++;
                }
            }
            else {
...
void opal_put_all_tokens(struct opal_token **tokens, size_t *len)
{
    if (*len >= OPAL_MAX_TOKENS) {
        SEDCLI_DEBUG_MSG("Put tokens error!\n");
        return;
    }
